...
def compute_loss(y, y_hat):
    ...
    loss = (1 - label) * -tf.math.log(tf.maximum(1 - pred, 1e-7)) + label * -tf.math.log(tf.maximum(pred, 1e-7))
...
def prepare_dataset(X, y):
    ...
    '''
    # shuffle the data
    X_trains, y_trains = shuffle(np.array(X_trains), np.array(y_trains), random_state=5)
    X_tests, y_tests = shuffle(np.array(X_tests), np.array(y_tests), random_state=10)
    '''
...
data = pickle.load(open("slip_database_grose.pickle", "rb"))
    ...
    for epoch in range(0, epochs):
        for (X_batch_train, y_batch_train) in train_dataset:
            ...
            with tf.GradientTape() as tape:
                y_batch_hat = rnn_model(X_batch_train)  # TODO
                loss = compute_loss(y_batch_train, y_batch_hat)  # TODO
